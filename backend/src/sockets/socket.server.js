    ...
    socket.on("ai-message", async (messagePayload) => {
      ...
      */
      try {
        ...
        const stmMessageIds = chatHistory.map((m) => m._id.toString());
        const LTM = memory
          .filter((item) => !stmMessageIds.includes(item.messageId?.toString()))
          .map((item) => ({
            role: "user",
            parts: [
              {
                text: `Previous context: ${item.metadata.text}`,
              },
            ],
          }));
        ...
        const [aiResponseMessage, responseVectors] = await Promise.all([
          ...
          chatModel.findByIdAndUpdate(messagePayload.chat, {
