import { CircleUser, LogOut, LayoutDashboard, FileUser } from "lucide-react/dist/cjs/lucide-react";
...
const Dropdown = ({ onClose }) => {
  ...
  return (
    <div
      className="absolute top-10 left-4 z-50 w-58 overflow-hidden rounded-xl border border-border bg-card shadow-lg"
    ...
    >
      ...
      <button onClick={() => handleNav("/dashboard")}
        className="flex w-full cursor-pointer items-center gap-2.5 px-4 py-2.5 text-sm text-foreground transition hover:bg-muted">
        <LayoutDashboard size={15} /> Dashboard
      </button>

      <button onClick={() => handleNav("/resume-analyzer")}
        className="flex w-full cursor-pointer items-center gap-2.5 px-4 py-2.5 text-sm text-foreground transition hover:bg-muted">
